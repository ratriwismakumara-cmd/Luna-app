class LunaNutrition extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(color: cream, child: SingleChildScrollView(padding: EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('Nutrition', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: purple)),
      SizedBox(height: 4),
      Text('Fuel for growing bodies', style: TextStyle(fontSize: 14, color: lavender)),
      SizedBox(height: 20),
      Container(padding: EdgeInsets.all(20), decoration: BoxDecoration(gradient: LinearGradient(colors: [lavender, purple]), borderRadius: BorderRadius.circular(20)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Activity Today', style: TextStyle(fontSize: 13, color: soft)),
        Row(children: [
          Text('Medium', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Colors.white)),
          Spacer(),
          Text('~1,400 kcal', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: gold)),
        ]),
      ])),
      SizedBox(height: 20),
  ...
  Widget _meal(String emoji, String meal, String food, String kcal) {
    return Card(elevation: 0, color: Colors.white, margin: EdgeInsets.only(bottom: 10), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)), child: ListTile(leading: Text(emoji, style: TextStyle(fontSize: 28)), title: Text('$meal: $food', style: TextStyle(fontSize: 14)), subtitle: Text(kcal, style: TextStyle(color: Color(0xFF6B5FA6)))));
  }
}
