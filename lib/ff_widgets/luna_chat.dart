...
class _LunaChatState extends State<LunaChat> {
  ...
  Widget build(BuildContext context) {
    return Container(color: cream, child: Column(children: [
      Container(height: 48, color: Colors.white, child: ListView(scrollDirection: Axis.horizontal, padding: EdgeInsets.symmetric(horizontal: 12, vertical: 8), children: ['Crisis','Reflection','Milestone','Bonding','Wellness','Observation'].map((m) => Padding(padding: EdgeInsets.only(right:8), child: ActionChip(label: Text(m, style: TextStyle(fontSize:11)), backgroundColor: Color(0xFFB9A7D9).withOpacity(0.15), side: BorderSide.none, onPressed: (){}))).toList())),
